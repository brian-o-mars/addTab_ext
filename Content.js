...
button.addEventListener("click", async () => {
  // Create the new tab
  const newTab = await chrome.tabs.create({ index: 0, active: false });

  // Get information about the current tab and group
  const [currentTab] = await chrome.tabs.query({ currentWindow: true, active: true });
  const gId = currentTab.groupId;
  const tIndex = currentTab.index;

  // Add the new tab to the group
  await chrome.tabs.group({ tabIds: newTab.id, groupId: gId });

  // Move the new tab
  await chrome.tabs.move(newTab.id, { index: tIndex });
